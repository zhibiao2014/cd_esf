      ...
      <div class="p10">
        ...
        <div class="un1_box t_no w380 fr">
          <ul class="Flist clearfix p10">
            <li class="w_w"><em>总价：</em><span class="c1 f16">{{{$house->price}}}</span>万元</li>
            <li class="w_w"><em>联系人：</em><span class="c1 f16">{{{$house->contacts}}}</span></li>
            <li class="pr tslw"><em>单价：</em><span>{{{ceil($house->price * 10000/$house->construction_area)}}}元/平米</span></li>
            <li><em>户型：</em><span>{{{$house->room}}}室{{{$house->hall}}}厅{{{$house->bathroom}}}卫</span></li>
          ...
          <ul class="Flist clearfix p10">
            ...
            <li><em>可经营类别：</em><span>
              @if ( !empty($house->shop_manager_type) )
              @foreach ( $house->shop_manager_type as $key => $value )
              @if ( isset( $shop_manager_types[$value] ) )
              {{{ $key == 0 ? $shop_manager_types[$value]['name'] : ' , ' . $shop_manager_types[$value]['name'] }}}
              @endif
              @endforeach
              @endif</span></li>
      ...
      <div class="house_detail">
        ...
        <div class="describe mt10">
          <table width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            <tr>
              <td class="w-120">商铺类型：</td>
              <td>{{{ $types[$house->type_id]['name'] }}}</td>
              <td class="w-120">铺面类型：</td>
              <td>{{{ isset( $shop_face_types[$house->shop_face_type_id] ) ? $shop_face_types[$house->shop_face_type_id]['name'] : '' }}}</td>
            </tr>

            <tr>
              <td class="w-120">可经营类别：</td>
              <td colspan="3">
              @if ( !empty($house->shop_manager_type) )
              @foreach ( $house->shop_manager_type as $key => $value )
              @if ( isset( $shop_manager_types[$value] ) )

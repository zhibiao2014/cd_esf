...
<div class="guide mb10 pr">
  ...
  <a href="{{route('esf.shop.rent.index')}}"> 商铺 </a> &gt;
  <a href="javascript:void(0);"> 出租 </a>
  <a target="_blank" href="{{route('rent_shop.create')}}" class="btn-fabu pa">
...
<div class="un2_box w998 mr10" style="margin-top:-1px;">
  <div id="rentid_60" class="mt20 ml20">
    <form action="{{action('front\ShopController@rent_lists')}}" method="get">
  ...
  <div class="sou_com w45li pr">
    <ul class="clearfix esf_ss_box pt10 pb10">
      <li class="clearfix">
        ...
        <a href="{{action('front\ShopController@rent_lists', array_diff_key( $p, array('region' => 0) ))}}"><span{{ ( !isset($p['region']) || empty($p['region']) ) ? ' class="s2 cur"' : ' class="s2"' }}>不限</span></a>
        <div class="s3">
          @foreach ($regions as $region)
          @if ( $region['pid'] == 0 )
          <a href="{{action('front\ShopController@rent_lists', array_merge( $p, array('region' => $region['id']) ))}}"{{ ( isset($p['region']) && $p['region'] == $region['id']) ? ' class="cur"' : '' }}> {{{$region['name']}}} </a>
          @endif
          @endforeach
        </div>
      </li>
      <li class="clearfix">
        ...
        <a href="{{action('front\ShopController@rent_lists', array_diff_key( $p, array('price' => 0) ))}}"><span{{ ( !isset($p['price']) || empty($p['price']) ) ? ' class="s2 cur"' : ' class="s2"' }}>不限</span></a>
        <div class="s3">
          @foreach ( $price as $value )
          <a href="{{action('front\ShopController@rent_lists', array_merge( $p, array('price' => $value->price) ))}}"{{ ( isset($p['price']) && $p['price'] == $value->price ) ? ' class="cur"' : '' }}> {{{$value->name}}} </a>
          @endforeach
          ...
          <input class="iput_but1" id="price_filter" type="button" data-url="{{action('front\ShopController@rent_lists')}}" data-query='{{json_encode($p)}}' value="价格筛选">
        </div>
      </li>
      <li class="clearfix">
        <strong class="s1">面积：</strong>
        <a href="{{action('front\ShopController@rent_lists', array_diff_key( $p, array('area' => 0) ))}}"><span{{ ( !isset($p['area']) || empty($p['area']) ) ? ' class="s2 cur"' : ' class="s2"' }}>不限</span></a>
        <div class="s3">
          @foreach ( $areas as $value )
          <a href="{{action('front\ShopController@rent_lists', array_merge( $p, array('area' => $value->area) ))}}"{{ ( isset($p['area']) && $p['area'] == $value->area ) ? ' class="cur"' : '' }}> {{{$value->name}}} </a>
          @endforeach
        </div>
      </li>
      <li class="clearfix">
        <strong class="s1">类型：</strong>
        <a href="{{action('front\ShopController@rent_lists', array_diff_key( $p, array('type' => 0) ))}}"><span{{ ( !isset($p['type']) || empty($p['type']) ) ? ' class="s2 cur"' : ' class="s2"' }}>不限</span></a>
        <div class="s3">
          @foreach ( $types as $value )
          <a href="{{action('front\ShopController@rent_lists', array_merge( $p, array('type' => $value->id) ))}}"{{ ( isset($p['type']) && $p['type'] == $value->id ) ? ' class="cur"' : '' }}> {{{$value->name}}} </a>
          @endforeach
        </div>
      </li>
      <li class="clearfix">
        <strong class="s1">行业：</strong>
        <a href="{{action('front\ShopController@rent_lists', array_diff_key( $p, array('shop_manager_type' => 0) ))}}"><span{{ ( !isset($p['shop_manager_type']) || empty($p['shop_manager_type']) ) ? ' class="s2 cur"' : ' class="s2"' }}>不限</span></a>
        <div class="s3">
          @foreach ( $shop_manager_types as $value )
          <a href="{{action('front\ShopController@rent_lists', array_merge( $p, array('shop_manager_type' => $value->id) ))}}"{{ ( isset($p['shop_manager_type']) && $p['shop_manager_type'] == $value->id ) ? ' class="cur"' : '' }}> {{{$value->name}}} </a>
          @endforeach
        </div>
      </li>
    </ul>
...
<div class="w754 fl">
  <div class="tab_house clearfix">
    <a class="tab02" href="{{route('esf.shop.index')}}"><span>商铺出售</span></a>
    <a class="tab01" href="{{action('front\ShopController@rent_lists')}}"><span>商铺出租</span></a>
  </div>
  <div class="un2_box btno pr">
    ...
    <dl id="tips" class="titHouse updown">
      ...
      <dd id="rentid_64" class="black">
        ...
        <select name="s" id="sort_select" data-url="{{action('front\ShopController@rent_lists', array_merge($p, array( 's' => '', 'page' => 1)))}}">
        ...
        <a href="{{action('front\ShopController@rent_lists', array_merge( $p, array('s' => array( 'price', 'asc' ) ) ))}}"><span class="rank2">总价</span> </a>
        @else
        <a href="{{action('front\ShopController@rent_lists', array_merge( $p, array('s' => array( 'price', 'desc' ) ) ))}}"><span class="rank1">总价</span></a>
        @endif
        @if ( isset($sort) && $sort[0] == 'construction_area' && $sort[1] == 'desc' )
        <a href="{{action('front\ShopController@rent_lists', array_merge( $p, array('s' => array( 'construction_area', 'asc' ) ) ))}}"><span class="rank2">面积</span> </a>
        @else
        <a href="{{action('front\ShopController@rent_lists', array_merge( $p, array('s' => array( 'construction_area', 'desc' ) ) ))}}"><span class="rank1">面积</span></a>
        @endif
      </dd>
    </dl>
    <!--列表 start-->
    ...
    <div class="fclist clearfix">
      <a title="{{{$house->community_name}}}" target="_blank" href="{{route('esf.rent_shop.show', array('id' => $house->id))}}" class="pic_k"><span class="I_more_pic">{{{count($room_images)}}} 图</span><img alt="{{{$house->community_name}}}" src="{{empty($house->thumbnail) ? asset('assets/images/esf/moren.jpg') : $house->thumbnail}}"></a>
      <ul class="centxt">
        <li>
          <div class="fr"><span class="c11">{{{$house->price}}}</span> 元/月 </div>
          <a href="{{route('esf.rent_shop.show', array('id' => $house->id))}}" target="_blank" class="t3"> {{{$house->title}}}
    ...
    @else
    <div class="fclist clearfix">
      <a title="{{{$house->community_name}}}" target="_blank" href="{{route('esf.rent_shop.show', array('id' => $house->id))}}" class="pic_k"><span class="I_more_pic">{{{count($room_images)}}} 图</span><span class="mianshuiPic"></span><img alt="{{{$house->community_name}}}" src="{{empty($house->thumbnail) ? asset('assets/images/esf/moren.jpg') : $house->thumbnail}}"></a>
      <ul class="centxt4">
        <li> <a href="{{route('esf.rent_shop.show', array('id' => $house->id))}}" target="_blank" class="t3"> {{{$house->title}}} </a> </li>
        <li>
...
<div class="w232 fr">
  ...
  <div class="un1_box">
    ...
    <div class="RecommendListings">
      ...
      <dl class="clearfix">
        <dt>
          <a title="{{{$value->community_name}}}" href="{{route('esf.rent_shop.show', array('id' => $value->id))}}"><img alt="{{{$value->community_name}}}" src="{{empty($value->thumbnail) ? asset('assets/images/esf/moren.jpg') : $value->thumbnail}}"></a>
        </dt>
        <dd>
          <h3 class="t4"> <a href="{{route('esf.rent_shop.show', array('id' => $value->id))}}">{{{$value->title}}}</a> </h3>

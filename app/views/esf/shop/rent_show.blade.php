  ...
  <div class="fl esfcenLt">
      ...
      <div class="p10">
        ...
        <div class="un1_box t_no w380 fr">
          <ul class="Flist clearfix p10">
            <li class="w_w"><em>租金：</em><span class="c1 f16">{{{$house->price}}}</span><?php $price_unit = array( '1' => '元/月', '2'  => '元/平米/天', '3' => '元/平米/月 ' ); ?></li>
            <li class="w_w"><em>联系人：</em><span class="c1 f16">{{{$house->contacts}}}</span></li>
            <li><em>类别：</em><span> {{{ $house->rent_type == 0 ? '商铺出租' : '商铺转让' }}} </span></li>
            <li><em>支付方式：</em><span>{{{ isset($pay_methods[$house->pay_method_id]) ? $pay_methods[$house->pay_method_id]['name'] : '面议' }}}</span></li>

            <li><em>面积：</em><span>{{{$house->construction_area}}}㎡</span></li>
            <li><em>商铺类型：</em><span>{{{isset($types[$house->type_id]) ? $types[$house->type_id]['name'] : '' }}}</span></li>

            <li><em>区域：</em><span>{{{isset($regions[$house->area_id]) ? $regions[$house->area_id]['name'] : ''}}}{{isset($regions[$house->region_id]) ? '-' . $regions[$house->region_id]['name'] : ''}} </span></li>
            <li><em>铺面类型：</em><span>{{{isset($shop_face_types[$house->shop_face_type_id]) ? $shop_face_types[$house->shop_face_type_id]['name'] : '' }}}</span></li>
    ...
    <div class="one_box mb10 p10">
      <div class="house_detail">
        <div class="tilte">
          <ul>
            <li id="anchor_des" style="padding-left: 0pt;" class="li_on">房源描述</li>
            <li class="li_line">|</li>
        ...
        <div class="describe mt10">
          <table>
            ...
            <tr>
              <td class="header">租金</td>
              <td>{{{$house->price}}}<?php $price_unit = array( '1' => '元/月', '2'  => '元/平米/天', '3' => '元/平米/月 ' ); ?></td>
              <td class="header">类别</td>
              <td>{{{ $house->rent_type == 0 ? '商铺出租' : '商铺转让' }}}</td>
            </tr>

            <tr>
              <td class="header">支付方式</td>
              <td>{{{ isset($pay_methods[$house->pay_method_id]) ? $pay_methods[$house->pay_method_id]['name'] : '面议' }}}</td>
              <td class="header">商铺类型</td>
              <td>{{{isset($types[$house->type_id]) ? $types[$house->type_id]['name'] : '' }}}</td>
            </tr>

            <tr>
              <?php $status = array( '1' => '营业中', '2' => '闲置中', '3' => '新铺'); ?>
              <td class="header">当前状态</td>
              <td>{{{isset($status[$house->status]) ? $status[$house->status] : '' }}}</td>
              <td class="header">铺面类型</td>
              <td>{{{isset($shop_face_types[$house->shop_face_type_id]) ? $shop_face_types[$house->shop_face_type_id]['name'] : '' }}}</td>
            </tr>

            <tr>
              <td class="header">区域</td>
            ...
            <tr>
              <td class="header">面积</td>
              <td>{{{ $house->construction_area }}}</td>
              <td class="header">建筑年代</td>
              <td>{{{$house->construct_year}}}</td>
            </tr>
            <tr>
              <td class="header">物业费</td>
              <td>{{{ $house->property_costs }}}元/月/月</td>
              <td class="header">物业公司</td>
              <td>{{{$house->property_corporation}}}</td>
            </tr>
            <tr>
              <td class="header">装修程度</td>
              <td>{{{isset($decorations[$house->decoration_id]) ? $decorations[$house->decoration_id]['name'] : '' }}}</td>
              <td class="header">有效期</td>
              <td>{{{ $house->validity }}}</td>
            </tr>
            <tr>
              <td class="header">房屋配套</td>
              <td colspan="3">
    ...
    <div class="one_box mb10 p10">
      <div class="house_detail">
        <div class="tilte">
          <ul>
            <li style="padding-left: 0pt;"><a href="#anchor_des" class="nofollow">房源描述</a></li>
            <li class="li_line">|</li>
    ...
    <div class="one_box mb10 p10">
      <div class="house_detail">
        <div class="tilte">
          <ul>
            <li id="esfshxq_122" style="padding-left: 0pt;"><a href="#anchor_des">房源描述</a></li>
            <li class="li_line">|</li>
            <li><a href="#anchor_pic" rel="nofollow">房源图片({{{count($house->room_images)}}})</a></li>
            <li class="li_line">|</li>

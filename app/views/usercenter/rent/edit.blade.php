...
<div class="set_cent fr">
  ...
  @endif
  {{ HTML::ul($errors->all() )}}
  {{ Form::model($rent, array('route' => array('rent.update', $rent['id']), 'method' => 'put' , 'id' => 'residence')) }}
  ...
  <div class="msglist mb20">
    <ul>
      <li>
        ...
        <input type="text" value="<?php echo $rent['contacts']; ?>" class="Input_1" name="info[contacts]" id="contacts"/>
      </li>
      <li class="phone">
        <label class="w127" for="phone"><span class="c1">*</span>手机号码：</label>
        <span class="origin_phone Input_1">{{$rent['phone']}}</span>
        <input type="text" name="info[phone]" value="{{$rent['phone']}}" class="Input_1 hidbox" id='phone' />
    ...
    <div class="msglist mb20">
      <ul>
        <li class="pr">
          ...
          <input type="text" maxlength="15" class="Input_1" style="width:300px;" id="community_name" name="info[community_name]" value="<?php echo $rent['community_name']; ?>" autocomplete="off" />
          <input type="hidden" name="info[community_id]" id="community_id" value="{{$rent['community_id']}}" />
        ...
        <li>
          ...
          <select autocomplete="off" class="input_7" name="info[region_id]" id="area_top">
            ...
            <?php if ($area['pid'] == 0 ) { ?>
            <option value="<?php echo $area['id']; ?>" <?php if( $area['id'] == $areas[$rent['area_id']]['pid'] ) { echo "selected"; } ?>><?php echo $area['name']; ?></option>
            <?php } ?>
            <?php } ?>
          </select>
          <select autocomplete="off" class="input_7" id="area" name="info[area_id]" >
            <option value="0">请选择</option>
            <?php foreach ($areas as $key => $area) { ?>
            <?php if ($area['pid'] == $areas[$rent['area_id']]['pid'] ) { ?>
            <option value="<?php echo $area['id']; ?>" <?php if( $area['id'] == $rent['area_id'] ) { echo "selected"; } ?>><?php echo $area['name']; ?></option>
            <?php } ?>
            <?php } ?>
          </select>
        </li>
        <li>
          ...
          {{ Form::text('info[address]', $rent['address'], array('class' => 'Input_1', 'style' => 'width:300px;', 'id' => 'address' )) }}
        </li>
        <li>
          ...
          <select class="input_7" name="info[rent_method_id]" id="rent_method_id">
            ...
            <option value="<?php echo $value['id']; ?>" <?php if( $value['id'] == $rent['rent_method_id'] ) { echo "selected"; } ?>><?php echo $value['name']; ?></option>
            <?php } ?>
          </select>
        </li>
        <li>
          <label class="w127" for="room_structure"><span class="c1">*</span>户型：</label>
          {{ Form::text('info[room_structure][room]', $rent['room_structure']['room'], array('class' => 'Input_1', 'style' => 'width:32px;', 'id' => 'input_House_Room' )) }}
          <span>室</span>
          {{ Form::text('info[room_structure][hall]', $rent['room_structure']['hall'], array('class' => 'Input_1', 'style' => 'width:32px;', 'id' => 'input_House_Hall' )) }}
          <span>厅</span>
          {{ Form::text('info[room_structure][bathroom]', $rent['room_structure']['bathroom'], array('class' => 'Input_1', 'style' => 'width:32px;', 'id' => 'input_House_Toilet' )) }}
          <span>卫</span>
        </li>
        <li>
          <label class="w127" for="construction_area"><span class="c1">*</span>建筑面积：</label>
          {{ Form::text('info[construction_area]', $rent['construction_area'], array('class' => 'Input_1', 'style' => 'width:120px;', 'id' => 'construction_area' )) }}平方米
        </li>
        <li>
          <label class="w127" for="price"><span class="c1">*</span>租金：</label>
          {{ Form::text('info[price]', $rent['price'], array('class' => 'Input_1', 'style' => 'width:120px;', 'id' => 'price' )) }} 元/月
        </li>
        <li>
          ...
          <select class="input_7" name="info[pay_method_id]" id="pay_method_id">
            ...
            <option value="<?php echo $method['id']; ?>" <?php if( $method['id'] == $rent['pay_method_id'] ) { echo "selected"; } ?>><?php echo $method['name']; ?></option>
            <?php } ?>
          </select>
        </li>
        <li>
          <label class="w127" for="input_House_Floor"><span class="c1">*</span>楼层：</label>
          {{ Form::text('info[floor][floor]', $rent['floor']['floor'], array('class' => 'Input_1', 'style' => 'width:32px;', 'id' => 'floor' )) }}
          层 共
          {{ Form::text('info[floor][total_floor]', $rent['floor']['total_floor'], array('class' => 'Input_1', 'style' => 'width:32px;', 'id' => 'total_floor' )) }}
          层
        </li>
        <li>
          <label class="w127" for="input_House_Block">楼 栋 号：</label>
          {{ Form::text('info[house_number][floor]', $rent['house_number']['floor'], array('class' => 'Input_1', 'style' => 'width:32px;', 'id' => 'input_House_Block' )) }}
          幢/号/层
          {{ Form::text('info[house_number][unit]', $rent['house_number']['unit'], array('class' => 'Input_1', 'style' => 'width:32px;', 'id' => 'input_House_UnitNumber' )) }}
          单元
          {{ Form::text('info[house_number][room]', $rent['house_number']['room'], array('class' => 'Input_1', 'style' => 'width:32px;', 'id' => 'input_House_RoomNum' )) }}
          室
        ...
        <li>
          ...
          <select class="input_7" name="info[direction_id]" id="slt_House_Forward">
            ...
            <option value="<?php echo $direction['id']; ?>"<?php if( $direction['id'] == $rent['direction_id'] ) {echo " selected";} ?>><?php echo $direction['name']; ?></option>
            <?php } ?>
          </select>
          <select class="input_7" name="info[decoration_id]" id="slt_House_Fitment">
            <option value="">请选择</option>
            <?php foreach ($decorations as $key => $decoration) { ?>
            <option value="<?php echo $decoration['id']; ?>"<?php if( $decoration['id'] == $rent['decoration_id'] ) {echo " selected";} ?>><?php echo $decoration['name']; ?></option>
            <?php } ?>
          </select>
        </li>
        <li>
          ...
          <div class="contedit fl">
            <ul class="welfare" id="fuli">
              ...
              <li <?php if ( in_array($tag['id'], $rent['tag']) ) { echo ' class="active"'; } ?>>
                <label for="{{$tag['id']}}"><?php echo $tag['name']; ?></label>
                <input type="checkbox" id="{{$tag['id']}}" name="info[tag][]"<?php if ( in_array($tag['id'], $rent['tag']) ) { echo ' checked'; } ?> value="<?php echo $tag['id']; ?>"  autocomplete="off" />
              </li>
              <?php } ?>
            </ul>
            <ul class="brightspot" id="brightspot">
              <?php foreach ($rent['customer_tag'] as $key => $tag) { ?>
    ...
    <div class="msglist mb20">
      <ul>
        <li>
          ...
          <input type="text" maxlength="40" size="40" class="Input_1" style="width:300px;" id="title" name="info[title]" value="<?php echo $rent['title']; ?>" autocomplete="off" />
        </li>
        <li class="clearfix">
          <label class="w127 flptao">房屋配套： </label>
          <div class="frptao">
            <?php foreach ($house_supportings as $key => $supporting) { ?>
            <label class="w127 fwptcheck"><input type="checkbox" value="<?php echo $supporting['id']; ?>" name="info[supporting][]" <?php if (in_array($supporting['id'], $rent['supporting'])) { echo "checked"; } ?> autocomplete="off" /><?php echo $supporting['name']; ?></label>
            <?php } ?>
        ...
        <li class="clearfix">
          ...
          <div class="frptao">
            <script id="editor" type="text/plain" style="width:568px;height:120px;" name="info[content]"><?php echo $rent['content']; ?></script>
        ...
        <li class="clearfix">
          ...
          <div class="clearfix pre-z" id="">
            <div id="preview">
              <?php foreach ($rent['room_images'] as $key => $room_image) { ?>
              <div class="imgbox">
                <div class="w_upload">
            ...
            <div class="frptao clearfix">
              ...
              <p class="f12 c1 mt5">已上传图片<span class="c1 upload_count">{{count($rent['room_images'])}}</span>/10，支持多张上传，最多上传10张，单张照片不超过3M。真实且未经处理的照片更能吸引网友关注。</p><p>如果图片太大，请把图片发给客服QQ：421467754或者发送邮件到421467754@qq.com，附上房源的网址，我们代处理。</p>
            </div>
          </div>
        </li>
        <!-- <li class="commission">
          <div class="mb30 mt20 tc">
            <a class="wt_button {{ $rent['is_commissioned'] ? 'active' : '' }}" href="#" data-commission='1'><strong>快速委托</strong></a>
            <a class="wt_button {{ !$rent['is_commissioned'] ? 'active' : '' }}" href="#" data-commission='0'><strong>个人发布</strong></a>
            <input type="hidden" name="info[is_commissioned]" value="{{{$rent['is_commissioned']}}}" />
          </div>
        </li> -->
...
<script type="text/javascript">
  var GlobalParams = { 'upload_swf' : '<?php echo asset("assets/js/uploadify/uploadify.swf"); ?>', 'upload_url' : '<?php echo route("attachment.store"); ?>', 'image_num' : {{count($rent['room_images'])}} };
  ...
  var customer_tag = <?php echo count($rent['customer_tag']); ?>;

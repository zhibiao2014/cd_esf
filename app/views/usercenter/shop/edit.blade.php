...
<div class="set_cent fr">
  ...
  @endif
  {{ HTML::ul($errors->all() )}}
  {{ Form::model($shop, array('route' => array('shop.update', $shop['id']), 'method' => 'put' , 'id' => 'residence')) }}
  ...
  <div class="msglist mb20">
    <ul>
      <li>
        ...
        <input type="text" value="<?php echo $shop['contacts']; ?>" class="Input_1" name="info[contacts]" id="contacts"/>
      </li>
      <li class="phone">
        <label class="w127" for="phone"><span class="c1">*</span>手机号码：</label>
        <span class="origin_phone Input_1">{{$shop['phone']}}</span>
        <input type="text" name="info[phone]" value="{{$shop['phone']}}" class="Input_1 hidbox" id='phone' />
    ...
    <div class="msglist mb20">
      <ul>
        <li>
          ...
          <input type="text" maxlength="20" id="title" class="Input_1" name="info[title]" value="<?php echo $shop['title'] ?>" autocomplete="off" />
        </li>
        <li class="pr">
          ...
          <input type="hidden" name="info[community_id]" id="community_id" value="{{$shop['community_id']}}" />
          <ul id="lp_results" class="lp_results pa" style="display: none;">
          </ul>
        </li>
        <li>
          <label class="w127" for="address"><span class="c1">*</span>地址：</label>
          {{ Form::text('info[address]', $shop['address'], array('class' => 'Input_1', 'style' => 'width:300px;', 'id' => 'address' )) }}
        </li>
        <li>
          ...
          <select autocomplete="off" class="input_7" name="info[region_id]" id="area_top">
            ...
            <?php if ($area['pid'] == 0 ) { ?>
            <option value="<?php echo $area['id']; ?>" <?php if( $area['id'] == $areas[$shop['area_id']]['pid'] ) { echo "selected"; } ?>><?php echo $area['name']; ?></option>
            <?php } ?>
            <?php } ?>
          </select>
        ...
        <li>
          ...
          <select class="input_7" name="info[type_id]" id="type">
            ...
            <option value="<?php echo $type['id']; ?>" <?php if( $type['id'] == $shop['type_id'] ) { echo "selected"; } ?>><?php echo $type['name']; ?></option>
            <?php } ?>
          </select>
        </li>
        <li>
          ...
          <select class="input_7" name="info[shop_face_type_id]" id="shop_face_type_id">
            ...
            <option value="<?php echo $type['id']; ?>" <?php if( $type['id'] == $shop['shop_face_type_id'] ) { echo "selected"; } ?>><?php echo $type['name']; ?></option>
            <?php } ?>
          </select>
        </li>
        ...
        <li class="clearfix">
          ...
          <div class="frptao">
            ...
            <label class="w127 fwptcheck"><input type="checkbox" value="<?php echo $value['id']; ?>" name="info[shop_manager_type][]" <?php if (in_array($value['id'], $shop['shop_manager_type'])) { echo "checked"; } ?> autocomplete="off" /><?php echo $value['name']; ?></label>
            <?php } ?>
        ...
        <li>
          ...
          {{ Form::text('info[price]', $shop['price'], array('class' => 'Input_1', 'style' => 'width:120px;', 'id' => 'price' )) }} 万元/套
        </li>
        <li>
          <label class="w127" for="construction_area"><span class="c1">*</span>建筑面积：</label>
          {{ Form::text('info[construction_area]', $shop['construction_area'], array('class' => 'Input_1', 'style' => 'width:120px;', 'id' => 'construction_area' )) }}平方米
        </li>
        <li>
          <label class="w127" for="construct_year">建筑年代：</label>
          {{ Form::text('info[construct_year]', $shop['construct_year'], array('class' => 'Input_1', 'style' => 'width:120px;', 'id' => 'construct_year' )) }} 年
        </li>
        <li>
          ...
          <select class="input_7" name="info[decoration_id]" id="slt_House_Fitment">
            ...
            <option value="<?php echo $decoration['id']; ?>"<?php if( $decoration['id'] == $shop['decoration_id'] ) {echo " selected";} ?>><?php echo $decoration['name']; ?></option>
            <?php } ?>
          </select>
        </li>
        <li class="clearfix">
          ...
          <div class="frptao">
            ...
            <label class="w127 fwptcheck"><input type="checkbox" value="<?php echo $supporting['id']; ?>" name="info[supporting][]" <?php if (in_array($supporting['id'], $shop['supporting'])) { echo "checked"; } ?> autocomplete="off" /><?php echo $supporting['name']; ?></label>
            <?php } ?>
        ...
        <li>
          ...
          <select id="validity" class="input_7" name="info[validity]">
            <option value="15" <?php if( 15 == $shop['validity'] ) {echo " selected";} ?>>15</option>
            <option value="30"<?php if( 30 == $shop['validity'] ) {echo " selected";} ?>>30</option>
            <option value="60"<?php if( 60 == $shop['validity'] ) {echo " selected";} ?>>60</option>
          </select>
          <span>天</span>
        </li>
        <li>
          ...
          <div class="contedit fl">
            <ul class="welfare" id="fuli">
              ...
              <li <?php if ( in_array($tag['id'], $shop['tag']) ) { echo ' class="active"'; } ?>>
                ...
                <input type="checkbox" id="{{$tag['id']}}" name="info[tag][]"<?php if ( in_array($tag['id'], $shop['tag']) ) { echo ' checked'; } ?> value="<?php echo $tag['id']; ?>"  autocomplete="off" />
              </li>
              <?php } ?>
            </ul>
    ...
    <div class="msglist mb20">
      <ul>
        <li class="clearfix">
          ...
          <textarea style="width:568px;height:120px;" name="info[traffic]"><?php echo \lib\Tool::br2nl($shop['traffic']); ?></textarea>
        </li>
        <li class="clearfix">
          <label class="w127 flptao" for="input_DESCRIPTION">周边配套：</label>
          <textarea style="width:568px;height:120px;" name="info[around]"><?php echo \lib\Tool::br2nl($shop['around']); ?></textarea>
        </li>
        <li class="clearfix">
          <label class="w127 flptao" for="input_DESCRIPTION">写字楼描述：</label>
          <textarea style="width:568px;height:120px;" name="info[content]"><?php echo \lib\Tool::br2nl($shop['content']); ?></textarea>
        </li>
        <li class="clearfix">
          ...
          <div class="clearfix pre-z" id="">
            ...
            <div class="frptao clearfix">
              ...
              <p class="f12 c1 mt5">已上传图片<span class="c1 upload_count">{{count($shop['room_images'])}}</span>/10，支持多张上传，最多上传10张，单张照片不超过3M。真实且未经处理的照片更能吸引网友关注。</p><p>如果图片太大，请把图片发给客服QQ：421467754或者发送邮件到421467754@qq.com，附上房源的网址，我们代处理。</p>
            </div>
          </div>
        </li>
        ...
        <!-- <li class="commission">
          <div class="mb30 mt20 tc">
            <a class="wt_button {{ $shop['is_commissioned'] ? 'active' : '' }}" href="#" data-commission='1'><strong>快速委托</strong></a>
            <a class="wt_button {{ !$shop['is_commissioned'] ? 'active' : '' }}" href="#" data-commission='0'><strong>个人发布</strong></a>
            <input type="hidden" name="info[is_commissioned]" value="{{{$shop['is_commissioned']}}}" />
          </div>
        </li> -->
...
<script type="text/javascript">
  var GlobalParams = { 'upload_swf' : '<?php echo asset("assets/js/uploadify/uploadify.swf"); ?>', 'upload_url' : '<?php echo route("attachment.store"); ?>', 'image_num' : {{count($shop['room_images'])}} };

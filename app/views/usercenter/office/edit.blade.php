...
<div class="set_cent fr">
  ...
  @endif
  {{ HTML::ul($errors->all() )}}
  {{ Form::model($office, array('route' => array('office.update', $office['id']), 'method' => 'put' , 'id' => 'residence')) }}
  ...
  <div class="msglist mb20">
    <ul>
      <li>
        ...
        <input type="text" value="<?php echo $office['contacts']; ?>" class="Input_1" name="info[contacts]" id="contacts"/>
      </li>
      <li class="phone">
        <label class="w127" for="phone"><span class="c1">*</span>手机号码：</label>
        <span class="origin_phone Input_1">{{$office['phone']}}</span>
        <input type="text" name="info[phone]" value="{{$office['phone']}}" class="Input_1 hidbox" id='phone' />
    ...
    <div class="msglist mb20">
      <ul>
        <li>
          ...
          <input type="text" maxlength="20" id="title" class="Input_1" name="info[title]" value="<?php echo $office['title'] ?>" autocomplete="off" />
        </li>
        <li class="pr">
          <label class="w127" for="community_name"><span class="c1">*</span>写字楼名称：</label>
          <input type="text" maxlength="15" class="Input_1" style="width:300px;" id="community_name" name="info[community_name]" value="<?php echo $office['community_name']; ?>" autocomplete="off"/>
          <input type="hidden" name="info[community_id]" id="community_id" value="{{$office['community_id']}}" />
          <ul id="lp_results" class="lp_results pa" style="display: none;">
          </ul>
        </li>
        <li>
          <label class="w127" for="address"><span class="c1">*</span>地址：</label>
          {{ Form::text('info[address]', $office['address'], array('class' => 'Input_1', 'style' => 'width:300px;', 'id' => 'address' )) }}
        </li>
        <li>
          ...
          <select autocomplete="off" class="input_7" name="info[region_id]" id="area_top">
            ...
            <?php if ($area['pid'] == 0 ) { ?>
            <option value="<?php echo $area['id']; ?>" <?php if( $area['id'] == $areas[$office['area_id']]['pid'] ) { echo "selected"; } ?>><?php echo $area['name']; ?></option>
            <?php } ?>
            <?php } ?>
          </select>
          <select autocomplete="off" class="input_7" id="area" name="info[area_id]" >
            <option value="0">请选择</option>
            <?php foreach ($areas as $key => $area) { ?>
            <?php if ($area['pid'] == $areas[$office['area_id']]['pid'] ) { ?>
            <option value="<?php echo $area['id']; ?>" <?php if( $area['id'] == $office['area_id'] ) { echo "selected"; } ?>><?php echo $area['name']; ?></option>
            <?php } ?>
            <?php } ?>
          </select>
        </li>
        <li>
          ...
          <select class="input_7" name="info[type_id]" id="type">
            ...
            <option value="<?php echo $type['id']; ?>" <?php if( $type['id'] == $office['type_id'] ) { echo "selected"; } ?>><?php echo $type['name']; ?></option>
            <?php } ?>
          </select>
        </li>
        <li>
          <label class="w127" for="price"><span class="c1">*</span>总价：</label>
          {{ Form::text('info[price]', $office['price'], array('class' => 'Input_1', 'style' => 'width:120px;', 'id' => 'price' )) }} 万元
        </li>
        <li>
          <label class="w127" for="input_House_Floor"><span class="c1">*</span>楼层：</label>
          {{ Form::text('info[floor][floor]', $office['floor']['floor'], array('class' => 'Input_1', 'style' => 'width:32px;', 'id' => 'floor' )) }}
          层 共
          {{ Form::text('info[floor][total_floor]', $office['floor']['total_floor'], array('class' => 'Input_1', 'style' => 'width:32px;', 'id' => 'total_floor' )) }}
          层
        </li>
        <li>
          <label class="w127" for="construction_area"><span class="c1">*</span>建筑面积：</label>
          {{ Form::text('info[construction_area]', $office['construction_area'], array('class' => 'Input_1', 'style' => 'width:120px;', 'id' => 'construction_area' )) }}平方米
        </li>
        <li>
          <label class="w127" for="construct_year">建筑年代：</label>
          {{ Form::text('info[construct_year]', $office['construct_year'], array('class' => 'Input_1', 'style' => 'width:120px;', 'id' => 'construct_year' )) }} 年
        </li>
        <li>
          <label class="w127" for="price">物业费：</label>
          {{ Form::text('info[property_costs]', $office['property_costs'], array('class' => 'Input_1', 'style' => 'width:120px;', 'id' => 'property_costs' )) }} 元/平米·月 年
        </li>
        <li>
          <label class="w127" for="price">物业公司：</label>
          {{ Form::text('info[property_corporation]', $office['property_corporation'], array('class' => 'Input_1', 'style' => 'width:120px;', 'id' => 'property_corporation' )) }}
        </li>
        <li>
          ...
          <select class="input_7" name="info[decoration_id]" id="slt_House_Fitment">
            ...
            <option value="<?php echo $decoration['id']; ?>"<?php if( $decoration['id'] == $office['decoration_id'] ) {echo " selected";} ?>><?php echo $decoration['name']; ?></option>
            <?php } ?>
          </select>
        </li>
        <li class="clearfix">
          ...
          <div class="frptao">
            ...
            <label class="w127 fwptcheck"><input type="checkbox" value="<?php echo $supporting['id']; ?>" name="info[supporting][]" <?php if (in_array($supporting['id'], $office['supporting'])) { echo "checked"; } ?> autocomplete="off" /><?php echo $supporting['name']; ?></label>
            <?php } ?>
        ...
        <li>
          ...
          <select id="validity" class="input_7" name="info[validity]">
            <option value="15" <?php if( 15 == $office['validity'] ) {echo " selected";} ?>>15</option>
            <option value="30"<?php if( 30 == $office['validity'] ) {echo " selected";} ?>>30</option>
            <option value="60"<?php if( 60 == $office['validity'] ) {echo " selected";} ?>>60</option>
          </select>
          <span>天</span>
        </li>
        <li>
          ...
          <div class="contedit fl">
            <ul class="welfare" id="fuli">
              ...
              <li <?php if ( in_array($tag['id'], $office['tag']) ) { echo ' class="active"'; } ?>>
                <label for="{{$tag['id']}}"><?php echo $tag['name']; ?></label>
                <input id="{{$tag['id']}}" type="checkbox" name="info[tag][]"<?php if ( in_array($tag['id'], $office['tag']) ) { echo ' checked'; } ?> value="<?php echo $tag['id']; ?>"  autocomplete="off" />
              </li>
              <?php } ?>
            </ul>
            <ul class="brightspot" id="brightspot">
              <?php foreach ($office['customer_tag'] as $key => $tag) { ?>
    ...
    <div class="msglist mb20">
      <ul>
        <li class="clearfix">
          ...
          <textarea style="width:568px;height:120px;" name="info[traffic]"><?php echo \lib\Tool::br2nl($office['traffic']); ?></textarea>
        </li>
        <li class="clearfix">
          <label class="w127 flptao" for="input_DESCRIPTION">周边配套：</label>
          <textarea style="width:568px;height:120px;" name="info[around]"><?php echo \lib\Tool::br2nl($office['around']); ?></textarea>
        </li>
        <li class="clearfix">
          <label class="w127 flptao" for="input_DESCRIPTION">写字楼描述：</label>
          <textarea style="width:568px;height:120px;" name="info[content]"><?php echo \lib\Tool::br2nl($office['content']); ?></textarea>
        </li>
        <li class="clearfix">
          <label class="w127 flptao">上传图片：</label>
          <div class="clearfix pre-z" id="">
            <div id="preview">
              <?php foreach ($office['room_images'] as $key => $room_image) { ?>
              <div class="imgbox">
                <div class="w_upload">
            ...
            <div class="frptao clearfix">
              ...
              <p class="f12 c1 mt5">已上传图片<span class="c1 upload_count">{{count($office['room_images'])}}</span>/10，支持多张上传，最多上传10张，单张照片不超过3M。真实且未经处理的照片更能吸引网友关注。</p><p>如果图片太大，请把图片发给客服QQ：421467754或者发送邮件到421467754@qq.com，附上房源的网址，我们代处理。</p>
            </div>
          </div>
        </li>
        ...
        <!-- <li class="commission">
          <div class="mb30 mt20 tc">
            <a class="wt_button {{ $office['is_commissioned'] ? 'active' : '' }}" href="#" data-commission='1'><strong>快速委托</strong></a>
            <a class="wt_button {{ !$office['is_commissioned'] ? 'active' : '' }}" href="#" data-commission='0'><strong>个人发布</strong></a>
            <input type="hidden" name="info[is_commissioned]" value="{{{$office['is_commissioned']}}}" />
          </div>
        </li> -->
...
<script type="text/javascript">
  var GlobalParams = { 'upload_swf' : '<?php echo asset("assets/js/uploadify/uploadify.swf"); ?>', 'upload_url' : '<?php echo route("attachment.store"); ?>', 'image_num' : {{count($office['room_images'])}} };
  ...
  var customer_tag = <?php echo count($office['customer_tag']); ?>;
  $(function() {
    /* 表单验证 */
    ...
    $("#type").formValidator({onshow:"请选择类型",onfocus:"请选择类型"}).inputValidator({min:1,onerror:"类型必选区域"}).defaultPassed();;
    ...
    $("#input_House_Block").formValidator({onshow:"请填写具体楼号",onfocus:"请填写具体楼号"}).defaultPassed();;
    $("#input_House_UnitNumber").formValidator({onshow:"请填写具体楼号",onfocus:"请填写具体单元号"}).defaultPassed();;
